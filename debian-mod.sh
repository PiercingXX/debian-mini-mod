...
setup_gnome_customizations_autostart_from_repo() {
    ...
    sudo install -d -o "$username" -g "$username" "$autostart_dir" "$runner_dir" "$script_dir"
    sudo install -d -o "$username" -g "$username" "$home_dir/.local" "$home_dir/.local/share" "$home_dir/.local/bin"
    sudo cp -f "$script_src" "$script_dir/gnome-customizations.sh"
    sudo chmod +x "$script_dir/gnome-customizations.sh"
    sudo chown "$username":"$username" "$script_dir/gnome-customizations.sh"
